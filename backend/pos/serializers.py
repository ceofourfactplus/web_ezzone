...
class CustomerSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ['first_name', 'last_name', 'phone_number',
                  'gender', 'address', 'alley', 'house_number']
...
class OrderItemToppingSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'item', 'topping',
                  'total_price', 'amount', 'topping_set']


class OrderItemSerializer(serializers.ModelSerializer):
    product_set = ProductSerializer(many=True)
    order_topping_set = OrderItemToppingSerializer(
        many=True, read_only=True, source="orderitemtopping_set")
...
class OrderSerializer(serializers.ModelSerializer):
    order_item_set = OrderItemSerializer(
        many=True, read_only=True, source="orderitem_set")
    customer = CustomerSerializer(read_only=True, source="customer_set")

    class Meta:
        model = Order
        fields = ['id', 'status_delivery', 'total_price', 'total_amount',
                  'sale_channel', 'customer', 'descriptions', 'customer_id', 'customer', 'status_order', 'payment', 'order_item_set','create_by','update_by']
