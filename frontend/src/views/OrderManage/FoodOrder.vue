<template>
  <div>
    <nav-app @back="$router.push({ name: 'DashBoard' })">Food Order</nav-app>
...
export default {
  ...
  methods: {
    ...
    get_order() {
      api_pos.get("order/today/on-going/food").then((response) => {
    ...
    is_food(item) {
      if (item.topping_set != null) {
        return (
          item.topping_set.type_topping == 1 ||
          item.topping_set.type_topping == 3
        );
      }
      if (item.product_set != null) {
        return (
          item.product_set.type_product == 1 ||
          item.product_set.type_product == 3
        );
      }
    },
  },
